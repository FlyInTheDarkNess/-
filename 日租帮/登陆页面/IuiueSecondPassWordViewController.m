//
//  IuiueSecondPassWordViewController.m
//  日租帮
//
...
@interface IuiueSecondPassWordViewController ()<UITextFieldDelegate>
{
...
@implementation IuiueSecondPassWordViewController
...
- (void)viewDidLoad {
    ...
    FirstPassword.placeholder = @"新密码";
    SecondPassword.placeholder = @"再次确认密码";
...
-(IBAction)TurnToSuccess:(id)sender{
    ...
    __weak ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:URL_FORGET_PASSWORD]];
    ...
    [request setCompletionBlock:^{
        ...
        if (!error) {
            switch ([resultDict[@"code"] integerValue]) {
                case 1:
                {
                    [self.view makeToast:@"密码重置成功" duration:0.5f position:@"center"];
                        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        IuiueFindBackSuccessViewController *success = [[IuiueFindBackSuccessViewController alloc]init];
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if(timeout<=0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                //设置界面的按钮显示 根据自己需求设置
                NSLog(@"____%@",strTime);
                self.navigationItem.rightBarButtonItem.title =[NSString stringWithFormat:@"(%@)秒后可重发",strTime];

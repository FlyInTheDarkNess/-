...
#import "IuiueRegisterViewController.h"
#import "IuiueSecondRegisterViewController.h"

@interface IuiueRegisterViewController ()<UITextFieldDelegate>
{
...
@implementation IuiueRegisterViewController
/**
 *  初始化
 */
...
-(IBAction)sendMessage:(id)sender{
    ...
    __weak ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:URL_SEND_REGMESSAGE]];
        ...
        if (!error) {
            switch ([resultDict[@"code"] integerValue]) {
                case 1:{
                    ...
                    [self startTime];
                    IuiueSecondRegisterViewController *second = [[IuiueSecondRegisterViewController alloc]init];
                    ...
                    break;
                case 0:{
                    [self.view makeToast:@"该手机号已注册，请直接登陆" duration:1.0f position:@"center"];
